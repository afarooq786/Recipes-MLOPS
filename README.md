...
GCP access is needed for `dvc pull` / `dvc push`. Ask the project owner for access to the shared GCP project and DVC remote, then authenticate with the Google Cloud CLI.
...
| 4 | Versioning | `dvc.yaml` / `dvc.lock` | Tracks pipeline dependencies and generated data by hash for reproducibility. |
...
| 6 | Feature Engineering | `features/build_features.py` | Creates ingredient, cooking-time, dietary, nutrition, and related engineered features. |
...
- **Test split**: intentionally isolated from model selection and tuning for later production validation. The isolated test set was not used during the modeling experiments described below.
...
| Model | Repeated-CV ROC-AUC | Validation ROC-AUC |
|-------|---------------------|---------------------|
| Char Logistic | ≈ 0.6277 | ≈ 0.6976 |
| Word SVM | ≈ 0.6477 | ≈ 0.6682 |
| Ensemble | ≈ 0.6810 | ≈ 0.6903 |
...
The current selected version is assigned the **champion** alias.
...
> ```
>
> `scripts/run_full_pipeline.ps1` does this automatically — see [Running the Full Pipeline with One Script](#running-the-full-pipeline-with-one-script) below.
...
This file is the Git-tracked source of truth for the currently approved model and its benchmark performance. The current champion is the character-TF-IDF Logistic Regression + word-TF-IDF Linear SVM rank ensemble, selected using 5-fold cross-validation repeated 10 times (50 held-out folds).
...
3. Compares the candidate's `validation_roc_auc` against the champion's `selection_metric_value`.
4. Promotes only if the candidate improves the metric — unless this is the first promotion attempt against an empty local registry, in which case it promotes unconditionally (see the bootstrap exception above).
5. On promotion: registers a new MLflow model version, moves the champion alias, and updates `champion_config.json`.
...
### Layer 1: Data quality and system metrics
...
### Layer 3: Evidently statistical drift + rule-based quality checks
...
- The target is extremely imbalanced, with roughly 94–95% positive examples after deduplication. Accuracy is therefore misleading as a primary metric.
